...
da=none
tau_da=1.0
beta_discount_factor=0
reg_weight=1e-3
learning_rate=1e-3
n_layers=3
python3 run_dagcf.py --dataset=$dataset --model=$model --DA_sampling=$da --config_files=./configs/motivation_dagcf.yaml --seed=$seed --n_layers=$n_layers --reg_weight=$reg_weight --learning_rate=$learning_rate --gpu_id=$gpu_id --tau_da=$tau_da --beta_discount_factor=$beta_discount_factor
## BPR
: '
